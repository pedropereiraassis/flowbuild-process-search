...
import {
  queryKVProcess,
  queryProcessOnSearchTable,
} from '../db/flowbuildDataSource'
...
export async function searchProcessesKeyValue(req: Request, res: Response) {
  logger.info('Called searchProcessesKeyValue controller')

  try {
    const { query, limit } = req.body

    const results = await queryKVProcess({
      query: query ?? {},
      limit,
    })
    ...
    logger.error('searchProcessesKeyValue error', error)
